...
| File | Purpose |
|---|---|
| `http-client.ts` | `HttpClient`: URL build, auth, timeout, envelope unwrap, error mapping, retry, ETag cache, raw and conditional requests, redirect resolution |
...
| `etag-cache.ts` | `ETagStore` contract and the in-memory LRU; cache keys are namespaced by a credential fingerprint so two keys cannot share a tier-shaped body |
